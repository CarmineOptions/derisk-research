...
from shared.protocol_loan_entities.zklend import ZkLendLoanEntity
from shared.helpers import add_leading_zeros, get_symbol
from shared.blockchain_call import func_call
from shared.types import Prices, ZkLendCollateralTokenParameters, ZkLendDebtTokenParameters

from .values import ZKLEND_EVENTS_TO_METHODS, ZKLEND_MARKET
...
class ZkLendState(State):
    ...
    async def collect_token_parameters(self) -> None:
        ...
        for underlying_collateral_token_address in collateral_tokens:
            ...
            reserve_data = await func_call(
                addr=ZKLEND_MARKET,
                selector="get_reserve_data",
                calldata=[underlying_collateral_token_address],
            )
            collateral_token_address = add_leading_zeros(
                hex(reserve_data[2])
            )
        ...
        for underlying_debt_token_address in debt_tokens:
            ...
            debt_token_symbol = await get_symbol(
