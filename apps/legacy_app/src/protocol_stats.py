...
def get_general_stats(
    ...
    for state in states:
        ...
        if isinstance(state, src.hashstack_v0.HashstackV0State) or isinstance(
...
def get_supply_stats(
        ...
        for token in src.settings.TOKEN_SETTINGS:
            ...
            # TODO: zkLend, Nostra Alpha and Nostra Mainnet should be implemented similarly to Hashstack V0 and V1.
                ...
                (
                    addresses,
                    selector,
                ) = src.protocol_parameters.get_supply_function_call_parameters(
                    protocol=protocol,
    ...
    data["Total supply (USD)"] = sum(
        data[column]
        * prices[src.settings.TOKEN_SETTINGS[column.replace(" supply", "")].address]
