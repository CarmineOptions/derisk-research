...
def initializer(mock_zklend_state):
    ...
    with patch(
        "data_handler.handlers.loan_states.zklend.utils.InitializerDBConnector"
...
def test_set_last_loan_states_per_users_success(
    initializer, mock_zklend_state, sample_loan_state
):
...
def test_convert_float_to_decimal_valid_data(mock_zklend_initializer):
    ...
    test_data = {"ETH": 100.0, "USDC": 1000.0}
    ...
    assert result == {"ETH": Decimal("100.0"), "USDC": Decimal("1000.0")}
...
def test_set_last_loan_states_per_users_db_error(initializer, mock_zklend_state):
    """Test set_last_loan_states_per_users with database error."""
    initializer.db_connector.get_zklend_by_user_ids.side_effect = SQLAlchemyError(
        "Database error"
    )
    initializer.zklend_state = mock_zklend_state
...
def test_convert_float_to_decimal_invalid_data(mock_zklend_initializer):
    ...
    initializer._convert_float_to_decimal.side_effect = lambda x: (
...
def test_set_last_loan_states_per_users_user_not_found(initializer, mock_zklend_state):
    ...
    initializer.db_connector.get_zklend_by_user_ids.assert_called_once_with(
        ["nonexistent_user"]
    )
