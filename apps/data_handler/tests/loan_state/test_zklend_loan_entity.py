...
from shared.loan_entity import LoanEntity, ZkLendLoanEntity
...
def zklend_loan_entity():
    ...
    with (
        patch.object(LoanEntity, "compute_collateral_usd") as mock_collateral_usd,
...
def mock_token_parameters():
    ...
    return {"ETH": eth_params, "USDC": usdc_params}


@pytest.fixture
def mock_prices():
    """Fixture providing mock token prices."""
    return {"ETH": decimal.Decimal("2000"), "USDC": decimal.Decimal("1")}


@pytest.fixture
def mock_interest_rate_models():
    """Fixture providing mock interest rate models."""
    return {"ETH": decimal.Decimal("1.05"), "USDC": decimal.Decimal("1.02")}
...
class TestZkLendLoanEntity:
    ...
    def test_compute_health_factor_no_debt(self, zklend_loan_entity, mock_prices):
        ...
        health_factor = zklend_loan_entity.compute_health_factor(
            standardized=False, prices=mock_prices
        )

        assert health_factor == decimal.Decimal("Inf")

    def test_compute_health_factor_with_debt(
        self, zklend_loan_entity, mock_prices, mock_interest_rate_models
    ):
    ...
    def test_compute_health_factor_with_risk_adjustment(
        ...
        zklend_loan_entity,
        mock_prices,
        mock_token_parameters,
        mock_interest_rate_models,
    ):
    ...
    @pytest.mark.parametrize(
        "collateral_amount,debt_amount,health_factor,expected_result",
        [
    ...
    def test_is_liquidatable(
        ...
        collateral_usd = decimal.Decimal(str(collateral_amount)) * decimal.Decimal(
            "2000"
        )
    ...
    def test_negative_values(self, zklend_loan_entity):
        ...
        with patch.object(Portfolio, "increase_value") as mock_increase:
            ...
            with pytest.raises(ValueError):
                zklend_loan_entity.collateral.increase_value(
                    "ETH", decimal.Decimal("-1.0")
                )

            with pytest.raises(ValueError):
                zklend_loan_entity.debt.increase_value(
                    "USDC", decimal.Decimal("-1000.0")
                )
