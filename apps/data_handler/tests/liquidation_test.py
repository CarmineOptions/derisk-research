...
from data_handler.handlers.liquidable_debt.debt_handlers import (
    BaseDBLiquidableDebtDataHandler,
)
...
class TestBaseDBLiquidableDebtDataHandler:
    ...
    def test_available_protocols(self):
        ...
        """
        with patch(
            "data_handler.handlers.liquidable_debt.values.LendingProtocolNames",
            ["Nostra_alpha", "Nostra_mainnet", "zkLend"],
        ):
            assert BaseDBLiquidableDebtDataHandler.AVAILABLE_PROTOCOLS == [
                "Nostra_alpha",
                "Nostra_mainnet",
                "zkLend",
            ]
    ...
    def test_get_prices_range_valid_token(self):
        ...
        """
        with patch(
            "data_handler.handlers.settings.TOKEN_PAIRS",
            {"ETH": ("USDC", "USDT", "DAI")},
        ):
    ...
    def test_fetch_data(self):
        ...
        with patch("data_handler.db.crud.DBConnector") as MockDBConnector:
            ...
            # Call the method
            loan_data, interest_rate_models = self.handler.fetch_data(
                self=self.handler, protocol_name=protocol_name
            )

            # Assertions for returned data
            assert loan_data == mock_loans, "Loan data mismatch"
            assert (
                interest_rate_models == mock_interest_rates
            ), "Interest rate models mismatch"

            # Ensure DBConnector methods were called with correct arguments
            mock_db_connector.get_loans.assert_called_once_with(
                model=LoanState, protocol=protocol_name
            )
