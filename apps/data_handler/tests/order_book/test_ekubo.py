...
def test_calculate_order_book(order_book):
    ...
    row_data = {"tick": 100, "tick_spacing": 5, "liquidity": "1000000"}
...
def test_sort_ticks_by_asks_and_bids():
    ...
    liquidity_data = [{"tick": 90}, {"tick": 100}, {"tick": 110}]
    current_tick = 100
    asks, bids = EkuboOrderBook.sort_ticks_by_asks_and_bids(
        liquidity_data, current_tick
    )
    assert len(asks) == 1
    assert len(bids) == 2
...
@pytest.mark.parametrize(
    "tick,expected_sign",
    [(1000, 1), (-1000, -1), (0, 0)],  # positive tick  # negative tick  # zero tick
)
