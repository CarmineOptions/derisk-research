...
EVENT_MAPPING: Dict[str, Tuple[Callable, str]] = {
    ...
    "Liquidation": (ZklendDataParser.parse_liquidation_event, "save_liquidation_event"),
    "zklend::market::Market::Liquidation": (
        ZklendDataParser.parse_liquidation_event,
        "save_liquidation_event",
    ),
    "Repayment": (ZklendDataParser.parse_repayment_event, "save_repayment_event"),
    "zklend::market::Market::Repayment": (
        ZklendDataParser.parse_repayment_event,
        "save_repayment_event",
    ),
    "Borrowing": (ZklendDataParser.parse_borrowing_event, "save_borrowing_event"),
    "zklend::market::Market::Borrowing": (
        ZklendDataParser.parse_borrowing_event,
        "save_borrowing_event",
    ),
    "Deposit": (ZklendDataParser.parse_deposit_event, "save_deposit_event"),
    "zklend::market::Market::Deposit": (
        ZklendDataParser.parse_deposit_event,
        "save_deposit_event",
    ),
    "Withdrawal": (ZklendDataParser.parse_withdrawal_event, "save_withdrawal_event"),
...
class ZklendTransformer:
    ...
    def save_liquidation_event(
        self, event_name: str, block_number: int, event_data: LiquidationEventData
    ) -> None:
        """
    ...
    def save_borrowing_event(
        self, event_name: str, block_number: int, event_data: BorrowingEventData
    ) -> None:
        """
    ...
    def save_deposit_event(
        self, event_name: str, block_number: int, event_data: DepositEventData
    ) -> None:
        """
    ...
    def save_withdrawal_event(
        self, event_name: str, block_number: int, event_data: WithdrawalEventData
    ) -> None:
        """
    ...
    def save_collateral_enabled_disabled_event(
        self,
        event_name: str,
        block_number: int,
        event_data: CollateralEnabledDisabledEventData,
    ) -> None:
        """
        ...
        self.db_connector.create_collateral_enabled_disabled_event(
            ...
            event_name=event_name,
            block_number=block_number,
            event_data={"user": event_data.user, "token": event_data.token},
        )

    def save_repayment_event(
        self, event_name: str, block_number: int, event_data: RepaymentEventData
    ) -> None:
        """
