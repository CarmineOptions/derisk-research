...
HASHSTACK_V1_SPECIFIC_TOKEN_SETTINGS: dict[str, HashstackV1SpecificTokenSettings] = {
    ...
    "DAI": HashstackV1SpecificTokenSettings(collateral_factor=decimal.Decimal("1"), debt_factor=decimal.Decimal("1")),
    "USDT": HashstackV1SpecificTokenSettings(collateral_factor=decimal.Decimal("1"), debt_factor=decimal.Decimal("1")),
    # TODO: Add wstETH.
...
TOKEN_SETTINGS: dict[str, TokenSettings] = {
    ...
    for token, token_settings in (src.settings.TOKEN_SETTINGS | HASHSTACK_V1_ADDITIONAL_TOKEN_SETTINGS).items()
}
...
class HashstackV1LoanEntity(src.state.LoanEntity):
    """
...
class HashstackV1State(src.state.State):
    ...
    def process_new_loan_event(self, event: pandas.Series) -> None:
        ...
        borrowed_collateral_token = src.helpers.get_symbol(src.helpers.add_leading_zeros(event["data"][5]))
        borrowed_collateral_face_amount = decimal.Decimal(str(int(event["data"][6], base=16)))
        original_collateral_token = self.ADDRESSES_TO_TOKENS[src.helpers.add_leading_zeros(event["data"][13])]
    ...
    def process_loan_spent_event(self, event: pandas.Series) -> None:
        ...
        new_debt_token = self.ADDRESSES_TO_TOKENS[src.helpers.add_leading_zeros(event["data"][14])]
    ...
    def process_loan_repaid_event(self, event: pandas.Series) -> None:
        ...
        new_borrowed_collateral_token = self.ADDRESSES_TO_TOKENS[src.helpers.add_leading_zeros(event["data"][17])]
        new_borrowed_collateral_face_amount = decimal.Decimal(str(int(event["data"][18], base=16)))
        new_original_collateral_token = self.ADDRESSES_TO_TOKENS[src.helpers.add_leading_zeros(event["data"][25])]
